(function () {
  ...
  async function applyProposal() {
    ...
    if (state.replacementTarget && state.replacementTarget.type === 'place') {
      var placeIndex = (fresh.savedPlaces || []).findIndex(function (item) { return String(item.id) === String(state.replacementTarget.id); });
      if (placeIndex < 0) return;
      var placeReplacement = Object.assign({}, selected[0], {
        id: fresh.savedPlaces[placeIndex].id,
        date: state.replacementTarget.date,
        time: state.replacementTarget.time,
        duration: state.replacementTarget.duration,
        autoGenerated: true,
  ...
  function openPlaceReplacement(placeId) {
    var trip = currentTrip();
    var place = trip && (trip.savedPlaces || []).find(function (item) { return String(item.id) === String(placeId); });
    if (!place) return;
    state.trip = trip;
    if (!dialog) buildDialog(trip);
    state.replacementTarget = { type: 'place', id: place.id, date: place.date, time: place.time || '09:30', duration: Number(place.duration || 90) };
    state.candidates = [];
    state.proposal = [];
    var form = dialog.querySelector('[data-auto-place-form]');
    form.hidden = false;
    form.querySelectorAll('[name="category"]').forEach(function (input) { input.checked = input.value === activityCategory(place.autoSearchCategory || place.category); });
    form.querySelectorAll('[name="date"]').forEach(function (input) { input.checked = input.value === place.date; });
    form.elements.perDay.value = '1';
    form.elements.pace.value = 'custom';
    form.elements.startTime.value = place.time || '09:30';
    form.elements.keepExisting.checked = true;
    form.elements.freeTerm.value = '';
    dialog.querySelector('#auto-place-title').textContent = 'החלפת מקום באמצעות חיפוש מלא';
    dialog.querySelector('[data-auto-place-status]').textContent = 'בחר סוג מקום והעדפות. היום והשעה הקיימים יישמרו.';
    dialog.querySelector('[data-auto-place-preview]').innerHTML = '';
    dialog.querySelector('[data-auto-place-preview-actions]').hidden = true;
    dialog.querySelector('[data-auto-place-apply]').innerHTML = '<i class="fa-solid fa-rotate"></i> החלף במקום שסומן';
    dialog.hidden = false;
    document.body.classList.add('auto-place-open');
    dialog.querySelector('[data-auto-place-close]').focus();
  }
  ...
  window.TravelMateAutoPlaces = { open: openDialog, replace: openPlaceReplacement, replaceQuick: replacementFor, replaceActivity: openActivityReplacement, replaceActivityQuick: replacementForActivity, replaceDay: replacementForDay };
