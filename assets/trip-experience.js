(function () {
  ...
  function createBudgetTools() {
    ...
    form.onsubmit = async function (event) { event.preventDefault(); var file = selectedReceiptFile(form); var status = panel.querySelector('[data-receipt-status]'); if (file && file.size > 5 * 1024 * 1024) { status.textContent = 'כדי לשמור את הקבלה בענן יש לבחור קובץ עד 5MB.'; return; } status.textContent = file ? 'שומר את הקבלה…' : ''; var receiptData = file ? await storedReceipt(file) : ''; if (file && !receiptData) { status.textContent = 'לא הצלחנו לשמור את הקובץ. נסה צילום או PDF אחר.'; return; } state.expenses.push({ id: Date.now(), amount: Number(form.amount.value), currency: form.currency.value, category: form.category.value, date: form.date.value, note: form.note.value.trim(), receiptName: file ? file.name : '', receiptType: file ? file.type : '', receiptSize: file ? file.size : 0, receiptData: receiptData }); writeJson(storageKey('expenses'), state.expenses); saveTripData(); form.reset(); form.date.value = localDateValue(); form.hidden = true; panel.querySelector('[data-receipt-preview]').hidden = true; status.textContent = ''; renderExpenseList(); renderBudgetColumns(); renderSummary(); toast('ההוצאה והקבלה נוספו לקטגוריית ' + state.expenses[state.expenses.length - 1].category + '.'); };
    renderExpenseList();
    renderBudgetCharts();
  }
  ...
  function renderExpenseList() {
    ...
    if (records) {
      records.innerHTML = state.expenses.length ? '<header><div><small>פירוט מלא</small><strong>הוצאות וקבלות שנשמרו</strong></div><b>' + state.expenses.length + '</b></header>' + state.expenses.slice().reverse().map(function (expense) {
        var hasReceipt = Boolean(expense.receiptData); var parts = expenseCategoryParts(expense.category); var receiptLabel = expense.receiptName || 'ללא קבלה מצורפת';
        return '<details class="expense-record" data-expense-id="' + escapeHtml(expense.id) + '"><summary><span class="expense-record-icon"><i class="fa-solid ' + (hasReceipt ? 'fa-receipt' : 'fa-wallet') + '"></i></span><span class="expense-record-title"><strong>' + escapeHtml(expense.note || 'הוצאה') + '</strong><small>' + escapeHtml(parts.category + (parts.subcategory ? ' / ' + parts.subcategory : '')) + ' · ' + escapeHtml(expense.date || 'ללא תאריך') + '</small></span><b>' + money(expense.amount, expense.currency || 'EUR') + '</b><i class="fa-solid fa-chevron-down expense-record-chevron"></i></summary><div class="expense-record-body"><dl><div><dt>מחיר כולל</dt><dd>' + money(expense.amount, expense.currency || 'EUR') + '</dd></div><div><dt>מטבע</dt><dd>' + escapeHtml(expense.currency || 'EUR') + '</dd></div><div><dt>קטגוריה</dt><dd>' + escapeHtml(expense.category || 'אחר') + '</dd></div><div><dt>תאריך</dt><dd>' + escapeHtml(expense.date || 'לא הוזן') + '</dd></div><div class="wide"><dt>קובץ קבלה</dt><dd>' + escapeHtml(receiptLabel) + '</dd></div></dl>' + (!hasReceipt && expense.receiptName ? '<p class="expense-receipt-warning">שם הקובץ הישן נשמר, אך תוכן הקבלה לא נשמר בעבר. יש לצרף אותה מחדש כדי לצפות או להוריד.</p>' : '') + '<div class="expense-record-actions">' + (hasReceipt ? '<button type="button" data-expense-view="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-eye"></i> צפייה בקבלה</button><button type="button" data-expense-download="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-download"></i> הורדת קבלה</button>' : '') + '<button type="button" class="danger" data-expense-delete="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-trash"></i> מחיקת הוצאה</button></div></div></details>';
      }).join('') : '<div class="expense-record-empty"><i class="fa-solid fa-receipt"></i><strong>עדיין אין קבלות</strong><span>קבלה שתסרוק ותשמור תופיע כאן עם כל הפרטים.</span></div>';
