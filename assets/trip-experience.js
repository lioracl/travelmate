(function () {
  ...
  async function receiptBlob(expense) {
    ...
    if (expense && expense.receiptData) return fetch(expense.receiptData).then(function (response) { return response.blob(); });
    return null;
  }
  ...
  function renderLocalBudgetTotals() {
    document.querySelectorAll('[data-budget]').forEach(function (node) {
      ...
      if (state.budgetUnlimited) {
        node.textContent = '\u05dc\u05dc\u05d0 \u05d4\u05d2\u05d1\u05dc\u05d4';
        if (parent) {
          parent.classList.add('is-unlimited-budget');
          if (parent.firstChild && parent.firstChild.nodeType === 3) parent.firstChild.nodeValue = '';
        }
        return;
      }
      if (parent) parent.classList.remove('is-unlimited-budget');
  ...
  function renderCurrency(failed) {
    document.querySelectorAll('[data-currency-insight]').forEach(function (host) {
      ...
      var unlimitedBudget = state.budgetUnlimited && Boolean(host.closest('#budget, .budget-card'));
  ...
  function createTotalBudgetEditor() {
    ...
    var modePicker = document.createElement('fieldset'); modePicker.className = 'budget-limit-mode';
    modePicker.innerHTML = '<legend>\u05e1\u05d5\u05d2 \u05d4\u05ea\u05e7\u05e6\u05d9\u05d1</legend><label><input type="radio" name="budgetMode" value="limited"><span><i class="fa-solid fa-wallet"></i><b>\u05ea\u05e7\u05e6\u05d9\u05d1 \u05de\u05d5\u05d2\u05d3\u05e8</b><small>\u05de\u05e2\u05e7\u05d1 \u05d0\u05d7\u05e8 \u05d1\u05d9\u05e6\u05d5\u05e2 \u05de\u05d5\u05dc \u05e1\u05db\u05d5\u05dd \u05d9\u05e2\u05d3</small></span></label><label><input type="radio" name="budgetMode" value="unlimited"><span><i class="fa-solid fa-infinity"></i><b>\u05dc\u05dc\u05d0 \u05d4\u05d2\u05d1\u05dc\u05d4</b><small>\u05d4\u05e1\u05db\u05d5\u05dd \u05d4\u05de\u05e6\u05d8\u05d1\u05e8 \u05e2\u05d5\u05dc\u05d4 \u05e2\u05dd \u05db\u05dc \u05d4\u05d5\u05e6\u05d0\u05d4</small></span></label>';
    form.prepend(modePicker);
    var euros = Number(state.trip.budget || inferBudget()); form.total.value = Math.round(state.localCurrency === 'EUR' ? euros : localFromEuros(euros));
    var settingsToggle = document.createElement('button'); settingsToggle.type = 'button'; settingsToggle.className = 'budget-settings-toggle'; settingsToggle.innerHTML = '<i class="fa-solid fa-sliders"></i><span>הגדרות תקציב</span><i class="fa-solid fa-chevron-down"></i>';
    var compactBudget = Boolean(window.matchMedia && window.matchMedia('(max-width:620px)').matches); form.hidden = compactBudget; settingsToggle.hidden = !compactBudget; settingsToggle.setAttribute('aria-expanded', String(!compactBudget)); settingsToggle.onclick = function () { form.hidden = !form.hidden; settingsToggle.setAttribute('aria-expanded', String(!form.hidden)); settingsToggle.classList.toggle('open', !form.hidden); if (!form.hidden) form.querySelector('input:not([type="radio"])').focus(); };
    hero.append(settingsToggle, form);
    function applyBudgetMode() { var unlimited = form.elements.budgetMode.value === 'unlimited'; form.classList.toggle('is-unlimited', unlimited); form.total.disabled = unlimited; form.total.required = !unlimited; }
    form.elements.budgetMode.value = state.budgetUnlimited ? 'unlimited' : 'limited'; applyBudgetMode();
    form.addEventListener('change', function (event) { if (event.target.name === 'budgetMode') applyBudgetMode(); });
    form.onsubmit = function (event) { event.preventDefault(); var unlimited = form.elements.budgetMode.value === 'unlimited'; state.budgetUnlimited = unlimited; if (!unlimited) { var localValue = Math.max(0, Number(form.total.value || 0)); var euroValue = state.localCurrency === 'EUR' ? localValue : (Number(state.rates[state.localCurrency] || 0) ? localValue / Number(state.rates[state.localCurrency]) : localValue); state.trip.budget = euroValue; document.querySelectorAll('[data-budget]').forEach(function (node) { node.dataset.euroAmount = String(euroValue); }); document.querySelectorAll('[data-currency-insight]').forEach(function (node) { node.dataset.euros = String(euroValue); }); } saveTripData(); renderLocalBudgetTotals(); renderCurrency(); renderBudgetCharts(); toast(unlimited ? '\u05d4\u05ea\u05e7\u05e6\u05d9\u05d1 \u05d4\u05d5\u05d2\u05d3\u05e8 \u05dc\u05dc\u05d0 \u05d4\u05d2\u05d1\u05dc\u05d4.' : '\u05d4\u05ea\u05e7\u05e6\u05d9\u05d1 \u05d4\u05db\u05d5\u05dc\u05dc \u05e0\u05e9\u05de\u05e8.'); };
  }
  function updateTotalBudgetEditor() { var form = document.querySelector('[data-total-budget-form]'); if (!form || document.activeElement === form.total) return; if (form.elements.budgetMode) form.elements.budgetMode.value = state.budgetUnlimited ? 'unlimited' : 'limited'; form.classList.toggle('is-unlimited', state.budgetUnlimited); form.total.disabled = state.budgetUnlimited; form.total.required = !state.budgetUnlimited; var euros = Number(state.trip && state.trip.budget || 0); var value = state.localCurrency === 'EUR' ? euros : localFromEuros(euros); if (value || !form.total.value) form.total.value = Math.round(value || euros); }
  ...
  function createBudgetTools() {
    ...
    panel.innerHTML = '<div class="expense-workspace-head"><div><small>מעקב מדויק</small><h2>הוצאות וקבלות</h2><p>צלם קבלה, סרוק אותה, בחר קטגוריה והוסף אותה ישירות לתקציב.</p></div><button type="button" data-expense-toggle><i class="fa-solid fa-camera"></i> צילום קבלה / הוצאה</button></div><div class="budget-columns-editor"><div><strong>עמודות התקציב</strong><small>אפשר לשנות שם וסכום, למחוק או להוסיף קטגוריה.</small></div><div data-budget-columns></div><button type="button" data-budget-column-add><i class="fa-solid fa-plus"></i> קטגוריה חדשה</button></div><form class="receipt-form" data-receipt-form hidden><div class="receipt-source-actions"><label class="receipt-source receipt-source-camera"><input name="receiptCamera" type="file" accept="image/*" capture="environment"><i class="fa-solid fa-camera"></i><span><strong>צילום במצלמה</strong><small>פתח את מצלמת הטלפון וצלם את הקבלה.</small></span></label><label class="receipt-source receipt-source-file"><input name="receiptFile" type="file" accept="image/*,application/pdf"><i class="fa-solid fa-file-arrow-up"></i><span><strong>העלאה מקובץ</strong><small>בחר תמונה או PDF שכבר שמורים במכשיר.</small></span></label></div><div class="receipt-preview" data-receipt-preview hidden></div><div class="receipt-grid"><label>כמה שולם?<input name="amount" type="number" min="0.01" step="0.01" required></label><label>מטבע<select name="currency"><option value="EUR">אירו (€)</option><option value="ILS">שקל (₪)</option><option value="USD">דולר ($)</option><option value="GBP">ליש״ט (£)</option></select></label><label>קטגוריה<select name="category" data-expense-category></select></label><label>תאריך<input name="date" type="date"></label><label class="wide">מה נקנה / בית עסק<input name="note" maxlength="160" required placeholder="לדוגמה: ארוחת ערב במסעדה"></label></div><p class="receipt-scan-status" data-receipt-status></p><div class="receipt-actions"><button type="button" data-receipt-scan><i class="fa-solid fa-wand-magic-sparkles"></i> סריקה חכמה</button><button type="submit"><i class="fa-solid fa-wallet"></i> הוסף לתקציב בקטגוריה</button></div></form><div class="expense-live-summary" data-expense-summary></div><section class="expense-records" data-expense-records aria-label="הוצאות וקבלות שנשמרו"></section>';
    ...
    function toggleForm() { var opening = form.hidden; form.hidden = !form.hidden; if (opening) { form.reset(); delete form.dataset.editingId; form.date.value = localDateValue(); form.currency.value = state.localCurrency; var status = panel.querySelector('[data-receipt-status]'); if (status) status.textContent = ''; panel.querySelector('[data-receipt-preview]').hidden = true; form.amount.focus(); } }
    panel.querySelector('[data-expense-toggle]').onclick = toggleForm; var legacyButton = section.querySelector('.section-head .pill-btn'); if (legacyButton) legacyButton.onclick = toggleForm; form.date.value = localDateValue();
    form.receiptCamera.onchange = function () { if (form.receiptCamera.files[0]) { form.receiptFile.value = ''; } previewReceipt(form.receiptCamera.files[0], panel); };
    form.receiptFile.onchange = function () { if (form.receiptFile.files[0]) { form.receiptCamera.value = ''; } previewReceipt(form.receiptFile.files[0], panel); };
    panel.querySelector('[data-receipt-scan]').onclick = function () { scanReceipt(form, panel); };
    ...
    form.onsubmit = async function (event) {
      ...
      var expenseId = editing ? editing.id : Date.now();
      var receiptPathValue = editing && editing.receiptPath || '';
      var receiptLocalKeyValue = editing && editing.receiptLocalKey || '';
      var receiptPendingValue = Boolean(editing && editing.receiptPending);
      var legacyReceiptData = editing && editing.receiptData || '';
      if (file) {
        try {
          var oldReceipt = editing && editing.receiptPath ? { receiptPath: editing.receiptPath } : null;
          receiptPathValue = await storePrivateReceiptWithRetry(file, expenseId);
          if (receiptLocalKeyValue) await deleteLocalReceipt(receiptLocalKeyValue);
          receiptLocalKeyValue = ''; receiptPendingValue = false;
          legacyReceiptData = '';
          ...
          try {
            ...
            console.warn('TravelMate receipt save failed', receiptError && receiptError.message || receiptError, localError && localError.message || localError);
            status.textContent = 'לא הצלחנו לשמור את קובץ הקבלה. פרטי ההוצאה לא נשמרו — נסה שוב או בחר קובץ קטן יותר.';
            return;
          }
        }
      }
      ...
      if (editing) state.expenses[state.expenses.indexOf(editing)] = saved; else state.expenses.push(saved);
      writeJson(storageKey('expenses'), state.expenses); saveTripData(); form.reset(); delete form.dataset.editingId; form.date.value = localDateValue(); form.hidden = true; panel.querySelector('[data-receipt-preview]').hidden = true; status.textContent = ''; renderExpenseList(); renderBudgetColumns(); renderSummary(); toast(saved.receiptPending ? 'ההוצאה נשמרה. הקבלה שמורה במכשיר וממתינה לסנכרון פרטי.' : editing ? 'ההוצאה עודכנה.' : 'ההוצאה והקבלה נוספו לקטגוריית ' + saved.category + '.');
    };
    renderExpenseList();
    renderBudgetCharts();
  }
  ...
  function storedReceipt(file) { if (file.type.indexOf('image/') === 0) return compressedReceipt(file); return new Promise(function (resolve) { var reader = new FileReader(); reader.onload = function () { resolve(String(reader.result || '')); }; reader.onerror = function () { resolve(''); }; reader.readAsDataURL(file); }); }
  function previewReceipt(file, panel) { var preview = panel.querySelector('[data-receipt-preview]'); if (!file) { preview.hidden = true; return; } if (preview.dataset.objectUrl) URL.revokeObjectURL(preview.dataset.objectUrl); var url = URL.createObjectURL(file); preview.dataset.objectUrl = url; preview.hidden = false; preview.innerHTML = '<img src="' + url + '" alt="תצוגה מקדימה של הקבלה"><span dir="auto" title="' + escapeHtml(file.name) + '">' + escapeHtml(file.name) + '</span>'; }
  ...
  function renderExpenseList() {
    ...
    if (records) {
      ...
      var grouped = state.expenses.reduce(function (result, expense) { var category = expenseCategoryParts(expense.category).category; (result[category] = result[category] || []).push(expense); return result; }, {});
      var categoryHtml = Object.keys(grouped).map(function (category) {
        var items = grouped[category].slice().reverse(); var categoryTotal = items.reduce(function (sum, expense) { return sum + expenseInEuros(expense); }, 0); var categoryLocal = state.localCurrency === 'EUR' ? categoryTotal : localFromEuros(categoryTotal);
        var rows = items.map(function (expense) {
          var hasReceipt = Boolean(expense.receiptPath || expense.receiptData || expense.receiptLocalKey); var parts = expenseCategoryParts(expense.category); var receiptLabel = expense.receiptName || 'ללא קבלה מצורפת'; var secondary = convertCurrency(Number(expense.amount || 0), expense.currency || 'EUR', state.secondaryCurrency);
          var secondaryText = secondary && state.secondaryCurrency !== (expense.currency || 'EUR') ? '<small class="expense-secondary">כ־' + money(secondary, state.secondaryCurrency) + '</small>' : '';
          return '<details class="expense-record" data-expense-id="' + escapeHtml(expense.id) + '"><summary><span class="expense-record-icon"><i class="fa-solid ' + (hasReceipt ? 'fa-receipt' : 'fa-wallet') + '"></i></span><span class="expense-record-title"><strong>' + escapeHtml(expense.note || 'הוצאה') + '</strong><small>' + escapeHtml(parts.subcategory || parts.category) + ' · ' + escapeHtml(expense.date || 'ללא תאריך') + '</small></span><span class="expense-record-price"><b>' + money(expense.amount, expense.currency || 'EUR') + '</b>' + secondaryText + '</span><i class="fa-solid fa-chevron-down expense-record-chevron"></i></summary><div class="expense-record-body"><dl><div><dt>מחיר כולל</dt><dd>' + money(expense.amount, expense.currency || 'EUR') + secondaryText + '</dd></div><div><dt>מטבע</dt><dd>' + escapeHtml(expense.currency || 'EUR') + '</dd></div><div><dt>קטגוריה</dt><dd>' + escapeHtml(expense.category || 'אחר') + '</dd></div><div><dt>תאריך</dt><dd>' + escapeHtml(expense.date || 'לא הוזן') + '</dd></div><div class="wide"><dt>קובץ קבלה</dt><dd class="expense-receipt-name">' + escapeHtml(receiptLabel) + (expense.receiptPending ? '<small> שמור במכשיר · ממתין לסנכרון פרטי</small>' : '') + '</dd></div></dl>' + (!hasReceipt && expense.receiptName ? '<p class="expense-receipt-warning">שם הקובץ הישן נשמר, אך תוכן הקבלה לא נשמר בעבר. יש לצרף אותה מחדש כדי לצפות או להוריד.</p>' : '') + '<div class="expense-record-actions"><button type="button" data-expense-edit="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-pen"></i> עריכת הוצאה</button>' + (hasReceipt ? '<button type="button" data-expense-view="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-eye"></i> צפייה בקבלה</button><button type="button" data-expense-download="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-download"></i> הורדת קבלה</button>' : '') + '<button type="button" class="danger" data-expense-delete="' + escapeHtml(expense.id) + '"><i class="fa-solid fa-trash"></i> מחיקת הוצאה</button></div></div></details>';
        }).join('');
        return '<details class="expense-category-group"><summary><span><i class="fa-solid fa-chevron-down"></i><strong>' + escapeHtml(category) + '</strong><small>' + items.length + ' הוצאות</small></span><b>' + money(categoryLocal, state.localCurrency) + '</b></summary><div class="expense-category-items">' + rows + '</div></details>';
      }).join('');
      records.innerHTML = state.expenses.length ? '<header><div><small>מתחת לתכנון מול ביצוע</small><strong>הוצאות וקבלות לפי קטגוריה</strong></div><label class="expense-secondary-picker"><span>מטבע משני</span><select data-expense-secondary-currency>' + codes.map(function (code) { return '<option value="' + code + '"' + (code === state.secondaryCurrency ? ' selected' : '') + '>' + code + '</option>'; }).join('') + '</select></label></header>' + categoryHtml : '<div class="expense-record-empty"><i class="fa-solid fa-receipt"></i><strong>עדיין אין קבלות</strong><span>קבלה שתסרוק ותשמור תופיע כאן לפי הקטגוריה שלה.</span></div>';
      var secondarySelect = records.querySelector('[data-expense-secondary-currency]'); if (secondarySelect) secondarySelect.onchange = function () { state.secondaryCurrency = secondarySelect.value; writeJson(storageKey('secondary-currency'), state.secondaryCurrency); saveTripData(); renderExpenseList(); };
      records.querySelectorAll('[data-expense-view]').forEach(function (button) { button.onclick = function () { openExpenseReceipt(button.dataset.expenseView); }; });
      records.querySelectorAll('[data-expense-download]').forEach(function (button) { button.onclick = function () { downloadExpenseReceipt(button.dataset.expenseDownload); }; });
      records.querySelectorAll('[data-expense-edit]').forEach(function (button) { button.onclick = function () { editExpense(button.dataset.expenseEdit); }; });
      records.querySelectorAll('[data-expense-delete]').forEach(function (button) { button.onclick = function () { deleteExpense(button.dataset.expenseDelete); }; });
    }
    renderBudgetCharts();
  }
