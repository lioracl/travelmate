(function () {
  ...
  function defaultBudgetCategories() {
    ...
    return [['לינה', .4], ['אוכל', .22], ['תחבורה', .15], ['אטרקציות', .13], ['רזרבה', .1]].map(function (item, index) { return { id: 'default-' + index, name: item[0], amount: Math.round(budget * item[1]) }; });
  }
  ...
  function renderBudgetColumns() {
    ...
    host.innerHTML = state.budgetCategories.map(function (item) { return '<div class="budget-column-row"><input data-budget-name="' + escapeHtml(item.id) + '" value="' + escapeHtml(item.name) + '" aria-label="שם קטגוריה"><label><span>' + escapeHtml(state.localCurrency) + '</span><input data-budget-amount="' + escapeHtml(item.id) + '" type="number" min="0" step="1" value="' + Number(item.amount || 0) + '" aria-label="סכום מתוכנן"></label><button type="button" data-budget-delete="' + escapeHtml(item.id) + '" aria-label="מחיקת קטגוריה"><i class="fa-solid fa-trash"></i></button></div>'; }).join('');
    if (select) { var selected = select.value; select.innerHTML = state.budgetCategories.map(function (item) { return '<option>' + escapeHtml(item.name) + '</option>'; }).join('') + '<option>אחר</option>'; if ([].slice.call(select.options).some(function (option) { return option.value === selected; })) select.value = selected; }
