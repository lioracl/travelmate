(function () {
  ...
  function currentPosition() { return new Promise(function (resolve, reject) { if (!window.isSecureContext) { reject(new Error('כדי להשתמש ב־GPS יש לפתוח את האפליקציה מאתר מאובטח. אפשר להשתמש בחיפוש בעיר היעד או במפה.')); return; } if (!navigator.geolocation) { reject(new Error('הדפדפן אינו תומך במיקום GPS. אפשר לחפש בעיר היעד או לבחור נקודה במפה.')); return; } var settled = false, timer = setTimeout(function () { if (!settled) { settled = true; reject(new Error('ה־GPS לא החזיר מיקום בזמן. אפשר לנסות שוב או להשתמש בחיפוש בעיר היעד.')); } }, 10000); navigator.geolocation.getCurrentPosition(function (position) { if (settled) return; settled = true; clearTimeout(timer); resolve(position); }, function (error) { if (settled) return; settled = true; clearTimeout(timer); var message = error.code === 1 ? 'לא ניתן אישור למיקום. אפשר לאפשר מיקום עבור TravelMate או הדפדפן בהגדרות הפרטיות של Android או iPhone.' : error.code === 3 ? 'קבלת המיקום ארכה יותר מדי. נסה שוב במקום פתוח או חפש בעיר היעד.' : 'המיקום אינו זמין כרגע. אפשר לחפש בעיר היעד או לבחור נקודה במפה.'; reject(new Error(message)); }, { enableHighAccuracy: false, timeout: 9000, maximumAge: 300000 }); }); }

  function requestGpsConsent(panel) { return new Promise(function (resolve) { try { if (sessionStorage.getItem('travelmate-gps-consent-mode') === 'session') { resolve('session'); return; } } catch (error) {} var old = panel.querySelector('[data-gps-consent]'); if (old) old.remove(); var card = document.createElement('section'); card.className = 'gps-consent-card'; card.dataset.gpsConsent = ''; card.innerHTML = '<i class="fa-solid fa-location-dot"></i><div><strong>איך לאפשר שימוש במיקום?</strong><p>TravelMate משתמשת במיקום רק לחיפוש מקומות קרובים. אין מעקב ברקע. לאחר הבחירה ייתכן שיופיע גם חלון ההרשאה של Android, iPhone או הדפדפן.</p><span><button type="button" data-gps-once>אישור חד־פעמי</button><button type="button" data-gps-session>אישור רק בזמן השימוש באפליקציה</button><button type="button" data-gps-deny>סירוב</button></span></div>'; panel.querySelector('[data-nearby-status]').insertAdjacentElement('afterend', card); function finish(value) { card.remove(); if (value === 'session') try { sessionStorage.setItem('travelmate-gps-consent-mode', 'session'); } catch (error) {} resolve(value); } card.querySelector('[data-gps-once]').addEventListener('click', function () { finish('once'); }); card.querySelector('[data-gps-session]').addEventListener('click', function () { finish('session'); }); card.querySelector('[data-gps-deny]').addEventListener('click', function () { finish('denied'); }); }); }
  ...
  panels.forEach(function (panel) {
    ...
    gpsButton.addEventListener('click', async function () { var permissionMode = await requestGpsConsent(panel); if (permissionMode === 'denied') { status.textContent = 'החיפוש לפי GPS בוטל. אפשר לחפש בעיר היעד ללא שיתוף מיקום.'; return; } gpsButton.disabled = true; status.classList.remove('is-error'); status.textContent = permissionMode === 'session' ? 'מקבל מיקום בזמן שהאפליקציה פתוחה…' : 'מקבל מיקום חד־פעמי מהמכשיר…'; results.innerHTML = ''; try { var position = await currentPosition(); status.textContent = 'המיקום התקבל. מחפש מקומות קרובים…'; await searchAt(position.coords.latitude, position.coords.longitude, position.coords.accuracy); } catch (error) { status.classList.add('is-error'); status.textContent = error.message; } finally { gpsButton.disabled = false; } });
