(function () {
  ...
  function renderPlaces(data, wikiPlaces, lat, lon, status, results, accuracy, warnings, searchTerm, category, destinationName) {
    ...
    results.innerHTML = googleSummary + resultToolsHtml(places.length) + places.map(function (place) { var distanceText = place.distance < 1000 ? Math.round(place.distance) + ' מ׳' : (place.distance / 1000).toFixed(1) + ' ק״מ', preciseQuery = [place.name, place.address, place.lat + ',' + place.lon].filter(Boolean).join(', '), googleMaps = googleMapsUrl(preciseQuery), osm = 'https://www.openstreetmap.org/?mlat=' + place.lat + '&mlon=' + place.lon + '#map=17/' + place.lat + '/' + place.lon, officialUrl = safeExternalUrl(place.url), infoScore = (place.image ? 2 : 0) + (officialUrl ? 2 : 0) + (place.address ? 1 : 0); return '<article class="nearby-result' + (place.image ? ' has-image' : '') + '" data-distance="' + Math.round(place.distance) + '" data-result-name="' + escapeHtml(place.name.toLowerCase()) + '" data-has-site="' + (officialUrl ? '1' : '0') + '" data-has-image="' + (place.image ? '1' : '0') + '" data-kosher="' + (place.isKosher ? '1' : '0') + '" data-info="' + infoScore + '">' + (place.image ? '<img src="' + escapeHtml(place.image) + '" alt="" loading="lazy">' : '') + '<div><span class="nearby-type">' + labels[place.type] + ' · ' + place.source + '</span><h3>' + escapeHtml(place.name) + '</h3><p>' + escapeHtml(place.description) + '</p><div class="nearby-links">' + (officialUrl ? '<a href="' + escapeHtml(officialUrl) + '" target="_blank" rel="noopener"><i class="fa-solid fa-globe"></i> אתר רשמי</a>' : '') + '<a class="google-rating-link" href="' + googleMaps + '" target="_blank" rel="noopener"><i class="fa-brands fa-google"></i> ציונים וביקורות</a><a href="' + osm + '" target="_blank" rel="noopener">מיקום ומקור</a></div></div><span class="nearby-distance">' + distanceText + '</span></article>'; }).join('');
