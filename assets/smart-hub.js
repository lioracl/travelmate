(function () {
  ...
  handlers.jetlag = function () { var host = panel('תוכנית Jet Lag', 'Mate יבנה תוכנית שינה, אור, קפה וארוחות לפני הטיסה ואחריה.', '<div class="smart-actions"><button class="primary" type="button" data-jetlag-ai>בנה תוכנית אישית</button></div>'); host.querySelector('[data-jetlag-ai]').onclick = function () { askAI('בנה לי תוכנית Jet Lag לטיול מישראל אל ' + state.trip.city + ', ' + state.trip.country + (state.trip.start ? ', יציאה ב' + dateLabel(state.trip.start) : '') + '. כלול שינה, חשיפה לאור, קפה וארוחות בשלושת הימים שלפני ואחרי ההגעה.'); }; };
  handlers.guide = function () { var intro = 'ברוך הבא ל' + state.trip.city + '. זה הזמן להאט, להביט סביב ולגלות את הסיפור המקומי של המקום.'; var host = panel('המדריך הקולי שלך', 'הקראה בעברית והכנת סיפור מפורט על המקום שבו אתה נמצא.', '<div class="smart-status">' + intro + '</div><div class="smart-actions"><button class="primary" type="button" data-guide-speak><i class="fa-solid fa-volume-high"></i> השמע</button><button type="button" data-guide-ai>הכן סיור קולי עם Mate</button></div>'); host.querySelector('[data-guide-speak]').onclick = function () { speak(intro); }; host.querySelector('[data-guide-ai]').onclick = function () { askAI('פעל כמדריך טיולים קולי ב' + state.trip.city + '. שאל אותי באיזה אתר אני נמצא ואז ספר עליו בעברית בצורה חיה, קצרה ומעניינת, כולל עובדה מפתיעה וטיפ מקומי.'); }; };
  function phrasePack(country) {
    var key = clean(country).toLowerCase();
    var packs = [
      { names: ['צכיה','צ׳כיה','czechia','czech republic'], language: 'צ׳כית', locale: 'cs-CZ', phrases: ['Dobrý den, děkuji.','Mohu platit kartou?','Potřebuji pomoc.','Kde je vlakové nádraží?','Mám potravinovou alergii.'] },
      { names: ['גרמניה','germany','אוסטריה','austria'], language: 'גרמנית', locale: 'de-DE', phrases: ['Guten Tag, danke.','Kann ich mit Karte bezahlen?','Ich brauche Hilfe.','Wo ist der Bahnhof?','Ich habe eine Lebensmittelallergie.'] },
      { names: ['איטליה','italy'], language: 'איטלקית', locale: 'it-IT', phrases: ['Buongiorno, grazie.','Posso pagare con la carta?','Ho bisogno di aiuto.','Dov’è la stazione ferroviaria?','Ho un’allergia alimentare.'] },
      { names: ['יפן','japan'], language: 'יפנית', locale: 'ja-JP', phrases: ['こんにちは、ありがとう。','カードで払えますか？','助けが必要です。','駅はどこですか？','食物アレルギーがあります。'] },
      { names: ['רומניה','romania'], language: 'רומנית', locale: 'ro-RO', phrases: ['Bună ziua, mulțumesc.','Pot plăti cu cardul?','Am nevoie de ajutor.','Unde este gara?','Am o alergie alimentară.'] },
      { names: ['צרפת','france'], language: 'צרפתית', locale: 'fr-FR', phrases: ['Bonjour, merci.','Puis-je payer par carte ?','J’ai besoin d’aide.','Où est la gare ?','J’ai une allergie alimentaire.'] },
      { names: ['ספרד','spain'], language: 'ספרדית', locale: 'es-ES', phrases: ['Hola, gracias.','¿Puedo pagar con tarjeta?','Necesito ayuda.','¿Dónde está la estación de tren?','Tengo una alergia alimentaria.'] },
      { names: ['יוון','greece'], language: 'יוונית', locale: 'el-GR', phrases: ['Γεια σας, ευχαριστώ.','Μπορώ να πληρώσω με κάρτα;','Χρειάζομαι βοήθεια.','Πού είναι ο σιδηροδρομικός σταθμός;','Έχω τροφική αλλεργία.'] },
      { names: ['פורטוגל','portugal'], language: 'פורטוגזית', locale: 'pt-PT', phrases: ['Olá, obrigado.','Posso pagar com cartão?','Preciso de ajuda.','Onde fica a estação de comboios?','Tenho uma alergia alimentar.'] },
      { names: ['הולנד','netherlands'], language: 'הולנדית', locale: 'nl-NL', phrases: ['Hallo, dank u.','Kan ik met de kaart betalen?','Ik heb hulp nodig.','Waar is het treinstation?','Ik heb een voedselallergie.'] },
      { names: ['הונגריה','hungary'], language: 'הונגרית', locale: 'hu-HU', phrases: ['Jó napot, köszönöm.','Fizethetek kártyával?','Segítségre van szükségem.','Hol van a vasútállomás?','Ételallergiám van.'] },
      { names: ['פולין','poland'], language: 'פולנית', locale: 'pl-PL', phrases: ['Dzień dobry, dziękuję.','Czy mogę zapłacić kartą?','Potrzebuję pomocy.','Gdzie jest dworzec kolejowy?','Mam alergię pokarmową.'] },
      { names: ['טורקיה','turkey','türkiye'], language: 'טורקית', locale: 'tr-TR', phrases: ['Merhaba, teşekkür ederim.','Kartla ödeyebilir miyim?','Yardıma ihtiyacım var.','Tren istasyonu nerede?','Gıda alerjim var.'] },
      { names: ['ישראל','israel'], language: 'עברית', locale: 'he-IL', phrases: ['שלום ותודה.','אפשר לשלם בכרטיס?','אני צריך עזרה.','איפה תחנת הרכבת?','יש לי אלרגיה למזון.'] },
      { names: ['בריטניה','אנגליה','united kingdom','uk','england'], language: 'אנגלית', locale: 'en-GB', phrases: ['Hello, thank you.','Can I pay by card?','I need help.','Where is the train station?','I have a food allergy.'] },
      { names: ['ארצות הברית','ארהב','ארה״ב','united states','usa'], language: 'אנגלית', locale: 'en-US', phrases: ['Hello, thank you.','Can I pay by card?','I need help.','Where is the train station?','I have a food allergy.'] }
    ];
    var found = packs.find(function (pack) { return pack.names.some(function (name) { return key.indexOf(name) !== -1; }); });
    return found || { language: 'השפה המקומית', locale: 'en-US', phrases: ['Hello, thank you.','Can I pay by card?','I need help.','Where is the train station?','I have a food allergy.'], fallback: true };
  }
  handlers.phrases = function () {
    var labels = ['שלום ותודה','אפשר לשלם בכרטיס?','אני צריך עזרה','איפה תחנת הרכבת?','יש לי אלרגיה למזון'];
    var pack = phrasePack(state.trip.country);
    var rows = labels.map(function (label, index) { return [label, pack.phrases[index]]; });
    var description = pack.fallback
      ? 'לא נמצאה עדיין חבילת שפה מובנית ל' + state.trip.country + '. המשפטים הבסיסיים באנגלית, וMate יכול להכין מיד משפטים בשפה המקומית.'
      : 'נבנה אוטומטית לפי ' + state.trip.country + '. הקש על משפט כדי לשמוע אותו ב' + pack.language + '.';
    var host = panel('משפטים שימושיים ב' + pack.language, description, '<div class="smart-list">' + rows.map(function (item) { return '<article><i class="fa-solid fa-language"></i><div><strong>' + escapeHtml(item[0]) + '</strong><span>' + escapeHtml(item[1]) + '</span></div><button type="button" data-speak="' + escapeHtml(item[1]) + '"><i class="fa-solid fa-volume-high"></i></button></article>'; }).join('') + '</div><div class="smart-actions"><button class="primary" type="button" data-phrase-ai>עוד משפטים עם Mate</button></div>');
    host.querySelectorAll('[data-speak]').forEach(function (button) { button.onclick = function () { speak(button.dataset.speak, pack.locale); }; });
    host.querySelector('[data-phrase-ai]').onclick = function () { askAI('צור לי פנקס של 15 משפטים שימושיים למטייל ב' + state.trip.country + '. הצג בעברית, בשפה המקומית ובתעתיק עברי קל להגייה.'); };
  };
